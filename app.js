...
function setupViewTabs() {
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (target === "trainers" && !trainersComputed && allRows.length) {
        buildTrainers(allRows);
      }
...
function renderDashboard(data) {
  ...
  // CST Gauge only (MSC gauge moved to Leading CSP view)
  renderGauge(
    "cstGauge",
    "cstGaugeLabel",
    data.cstPct,
...
function computeFeedbackKpis(coachingData, hoja1Data) {
  ...
  // Calculate MSC completion for managers
  let mscCompleted = 0;
  let mscTotal = 0;
  hoja1Data.forEach((r) => {
    const isManager = (r["Is Manager"] || "").toString().trim().toLowerCase();
  ...
  const mscPct = mscTotal > 0 ? (mscCompleted / mscTotal) * 100 : 0;
...
function renderFeedbackKpis(kpis) {
  ...
  renderFeedbackGauge(kpis.overallPct, kpis.employeesWithFeedback);
  renderMscGauge(kpis.mscPct, kpis.mscCompleted, kpis.mscTotal);
  renderFeedbackMap(kpis.countries);
}

function renderMscGauge(pct, completed, total) {
  const canvas = document.getElementById("mscGauge");
  const ctx = canvas.getContext("2d");
  const labelEl = document.getElementById("mscGaugeLabel");
...
function renderFeedbackGauge(pct, count) {
  const canvas = document.getElementById("feedbackGauge");
  const ctx = canvas.getContext("2d");
  const labelEl = document.getElementById("feedbackGaugeLabel");
  
  const clamped = Math.max(0, Math.min(100, pct || 0));
  labelEl.innerHTML = `<span class="gauge-number">${count}</span><span class="gauge-percent">${clamped.toFixed(1)}%</span>`;
...
// CST Trainers View
let trainersFilterInitialized = false;
...
function initTrainersFilters() {
  ...
  populateSelect("filterTrainersCountry", getUniqueSorted("Country"), "All countries");

  document.getElementById("filterTrainersCountry").addEventListener("change", applyTrainersFilters);
  document.getElementById("clearTrainersFiltersBtn").addEventListener("click", () => {
    document.getElementById("filterTrainersCountry").selectedIndex = 0;
    applyTrainersFilters();
  });
}

function applyTrainersFilters() {
  const countryFilter = document.getElementById("filterTrainersCountry").value;
  
  let filteredRows = allRows;
  if (countryFilter !== "__ALL__") {
    filteredRows = allRows.filter((r) => (r["Country"] ?? "").toString().trim() === countryFilter);
  }
  
  buildTrainers(filteredRows);
}

function buildTrainers(rows) {
  if (!trainersFilterInitialized && allRows.length) {
    initTrainersFilters();
    trainersFilterInitialized = true;
  }

  // Filter trainers - those with "Train The Trainers" column having a value
  const trainers = rows.filter((r) => {
    const ttt = (r["Train The Trainers"] || "").toString().trim();
    return ttt !== "";
  });

  // Update total trainers KPI
  document.getElementById("totalTrainers").textContent = trainers.length;

  // Count trainers by country
  const countryMap = new Map();
  trainers.forEach((r) => {
    const country = (r["Country"] || "").toString().trim();
    if (country) {
  ...
  // Build bar chart
  const ctx = document.getElementById("trainersBarChart")?.getContext("2d");
  if (ctx) {
    if (trainersBarChart) trainersBarChart.destroy();
    trainersBarChart = new Chart(ctx, {
      type: "bar",
      data: {
  ...
  if (listContainer) {
    ...
    trainers.forEach((r) => {
      ...
      item.innerHTML = `
        ...
        <span class="trainer-country">${country}</span>
      `;
      listContainer.appendChild(item);
    });
  }

  trainersComputed = true;
}
